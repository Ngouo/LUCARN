//#region node_modules/animejs/lib/anime.es.js
...
function hexToRgba(hexValue) {
	var hex = hexValue.replace(/^#?([a-f\d])([a-f\d])([a-f\d])$/i, function(m, r, g, b) {
		return r + r + g + g + b + b;
	});
...
function parseTargets(targets) {
	return filterArray(targets ? flattenArray(is.arr(targets) ? targets.map(toArray) : toArray(targets)) : [], function(item, pos, self) {
...
function setTargetsValue(targets, properties) {
	getAnimatables(targets).forEach(function(animatable) {
		for (var property in properties) {
			...
			var to = getRelativeValue(validateValue(value, valueUnit || getUnit(originalValue)), originalValue);
			setProgressValue[getAnimationType(target, property)](target, property, to, animatable.transforms, true);
		}
	});
}
...
function anime(params) {
	...
	instance.remove = function(targets) {
		removeTargetsFromInstance(parseTargets(targets), instance);
	};
	instance.reset();
...
function stagger(val, params) {
	...
	return function(el, i, t) {
		...
		return start + (isRange ? (val2 - val1) / maxValue : val1) * (Math.round(values[i] * 100) / 100) + unit;
	};
}
